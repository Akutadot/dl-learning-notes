...
Linux 是多用户操作系统，可以创建不同用户，并为不同用户分配不同权限。用户相关信息通常保存在 `/etc/passwd` 文件中，用户组相关信息通常保存在 `/etc/group` 文件中。

### 1.26.1 创建用户：useradd

创建用户可以使用 `useradd` 命令。普通用户通常没有权限直接创建用户，需要加 `sudo`。

```bash
sudo useradd 用户名
```

如果希望创建用户时自动创建用户主目录，可以使用 `-m`：
...
表示创建用户 `laowang`，并在 `/home` 下创建对应的用户主目录。

### 1.26.2 查看用户信息

可以通过查看 `/etc/passwd` 文件了解系统中的用户信息：
...
`/etc/passwd` 中每一行代表一个用户，常见字段包括：

```text
用户名:密码占位符:uid:gid:用户描述:用户主目录:shell
```

例如：

```text
laowang:x:1001:1001::/home/laowang:/bin/sh
```
...
- 第 1 个字段表示用户名；
- 第 2 个字段 `x` 表示密码占位符；
- 第 3 个字段表示用户 ID，即 uid；
- 第 4 个字段表示用户组 ID，即 gid；
- 第 6 个字段表示用户主目录；
- 第 7 个字段表示登录后使用的 shell。

### 1.26.3 设置用户密码：passwd

给用户设置密码可以使用 `passwd` 命令：
...
### 1.26.5 修改用户信息：usermod

`usermod` 可以修改用户信息，例如修改用户所属附加组。
...
表示把 `laowang` 加入 `sudo` 附加组，使其具备使用 `sudo` 的权限。
...
使用删除用户命令时要谨慎，尤其是带 `-r` 时，会同时删除用户主目录中的文件。

---

## 1.27 用户组相关操作

用户组用于管理一类用户的权限。Linux 中一个用户可以属于一个主组，也可以属于多个附加组。
...
注意：如果某个用户的主组仍然是该用户组，通常不能直接删除该用户组，需要先修改或删除相关用户。

---

### 1.27.5 使用 gpasswd 管理附加组

`gpasswd` 可以用于添加或删除用户的附加组。
...
第二天主要回顾并继续学习了 Linux 高级命令部分，包括用户与用户组管理、文件复制选项、远程登录、远程拷贝、vim 编辑器、软件安装和软件卸载等内容。

这一阶段的重点是从“本地文件操作”进一步过渡到“系统管理”和“远程服务器操作”。例如：

1. 使用 `useradd`、`passwd`、`su`、`usermod`、`userdel` 管理用户；
2. 使用 `groupadd`、`groupdel`、`gpasswd` 管理用户组；
3. 使用 `cp -a` 保留文件属性进行复制；
4. 使用 `ssh` 远程登录服务器；
5. 使用 `scp` 在本地和远程服务器之间传输文件；
6. 学习 vim 的基本模式和常用命令；
7. 学习使用 `dpkg` 和 `apt-get` 安装、卸载软件。

这些内容和后续深度学习服务器使用关系较大，尤其是 SSH 远程登录、远程拷贝和 Linux 权限管理。

---

## 1.29 cp 命令选项 -a 的使用

`cp` 命令用于复制文件或目录。之前已经学习过：
...
`cp -a` 中的 `-a` 可以理解为归档复制，常用于复制目录并尽量保留原文件属性。
...
表示将 `AA` 复制到 `BB`，并尽量保留 `AA` 中原有文件和目录的属性。

### 1.29.2 cp -r 和 cp -a 的区别

| 命令 | 作用 |
|---|---|
| `cp -r` | 递归复制目录 |
| `cp -a` | 递归复制目录，并尽量保留权限、时间、链接等属性 |

一般普通复制目录可以使用 `cp -r`，如果希望复制后尽量保持原目录结构和属性，可以使用 `cp -a`。

---

## 1.30 远程登录

远程登录主要使用 `ssh` 命令。SSH 可以让本地电脑连接远程 Linux 服务器，并在远程服务器上执行命令。
...
注意：`exit` 只是断开 SSH 连接，不等于关闭云服务器。如果使用 AutoDL 等云平台，停止计费需要在网页控制台中点击关机。

---

## 1.31 远程拷贝

远程拷贝主要使用 `scp` 命令。`scp` 可以在本地电脑和远程服务器之间传输文件或目录。

### 1.31.1 从本地复制文件到远程服务器
...
```bash
scp 1.txt user@192.168.1.100:/home/user/
```

如果远程服务器使用指定端口，需要加 `-P`，注意这里是大写 `P`：

```bash
scp -P 端口号 本地文件 用户名@服务器IP:远程路径
```

例如：

```bash
scp -P 33984 1.txt root@connect.weste.seetacloud.com:/root/autodl-tmp/
```

### 1.31.2 从远程服务器复制文件到本地
...
```bash
scp user@192.168.1.100:/home/user/1.txt ./
```

带端口的写法：

```bash
scp -P 33984 root@connect.weste.seetacloud.com:/root/autodl-tmp/1.txt ./
```
...
```bash
scp -P 33984 -r ./data root@connect.weste.seetacloud.com:/root/autodl-tmp/
```

### 1.31.4 学习理解

`ssh` 是远程登录，`scp` 是远程拷贝。二者都依赖 SSH 协议。后续使用云端 GPU 服务器时，常用 SSH 登录服务器，用 SCP 或 VS Code Remote-SSH 上传代码和数据。

---

## 1.32 vim 的介绍

`vim` 是 Linux 中常见的文本编辑器，是早期 `vi` 编辑器的增强版本。它功能强大，可以完全通过键盘命令完成编辑操作。

在 Linux 服务器中，很多时候没有图形界面，不能使用记事本、gedit 这类图形编辑器，因此需要掌握基本的 vim 操作。
...
如果文件存在，就打开该文件；如果文件不存在，保存后会创建新文件。
...
| 模式 | 作用 |
|---|---|
| 命令模式 | 默认模式，用于复制、粘贴、删除、移动光标等 |
| 编辑模式 | 用于输入和修改文本内容 |
| 末行模式 | 用于保存、退出、搜索、替换等操作 |
...
表示只替换第 1 行到第 10 行中的内容。

### 1.33.5 学习理解

vim 的核心是先分清模式。刚进入 vim 时不能直接输入内容，因为默认是命令模式；需要按 `i` 进入编辑模式。编辑完成后按 `Esc` 回到命令模式，再输入 `:wq` 保存退出。
...
1. 离线安装：使用 `.deb` 安装包；
2. 在线安装：使用 `apt-get` 从软件源下载安装。

---

### 1.34.1 deb 文件格式安装

`.deb` 是 Ubuntu/Debian 系统中常见的软件安装包格式，可以使用 `dpkg` 命令安装。
...
使用 `apt-get` 安装软件时，本质上是从软件源服务器下载软件包。如果默认服务器在国外，下载速度可能较慢，因此可以更换国内镜像源，例如阿里云镜像源、清华大学镜像源等。
...
Ubuntu 中软件卸载也可以使用 `dpkg` 或 `apt-get`。

---

### 1.35.1 dpkg 方式卸载

如果软件是通过 deb 安装包安装的，可以使用：
...
- `-r` 表示 remove，即卸载软件；
- 这里写的是软件包名，不一定是 `.deb` 文件名。

---

### 1.35.2 apt-get 方式卸载

如果软件是通过 `apt-get` 安装的，可以使用：
...
| 操作方式 | 安装 | 卸载 |
|---|---|---|
| deb 安装包 | `sudo dpkg -i 安装包.deb` | `sudo dpkg -r 软件包名` |
| apt-get 在线方式 | `sudo apt-get install 软件包名` | `sudo apt-get remove 软件包名` |

`dpkg` 更偏向本地 deb 包管理，`apt-get` 更偏向在线软件源管理。平时在 Ubuntu 中安装常用软件，更多使用 `apt-get install`。

---

## 1.36 Linux 第二阶段学习小结

本阶段继续学习了 Linux 用户管理、用户组管理、远程连接、远程拷贝、vim 编辑器和软件安装卸载等内容。
...
6. 使用 `groupadd` 创建用户组；
7. 使用 `groupdel` 删除用户组；
8. 使用 `gpasswd` 管理用户附加组；
9. 使用 `cp -a` 保留属性复制文件或目录；
10. 使用 `ssh` 远程登录 Linux 服务器；
11. 使用 `scp` 进行远程文件拷贝；
12. 理解 vim 的命令模式、编辑模式和末行模式；
13. 掌握 vim 中保存、退出、复制、粘贴、删除、搜索和替换等基本操作；
14. 使用 `dpkg` 安装和卸载 deb 软件包；
15. 使用 `apt-get` 在线安装和卸载软件。

这些内容和后续深度学习服务器使用关系较大，尤其是远程登录、远程拷贝、vim 编辑和软件安装。后续需要继续在实际操作中熟悉这些命令。
